/**
 ...
 A program to test the class Temperature.

 This program creates objects of class Temperature and
 exercises its methods.

 A Temperature object has two parameters:
		temperature value (a real number with a fractional part),
		and a type, either 'C' for Celsius or 'F' for Fahrenheit.

 Temperature's public member functions:
		Four constructors, one to specify both parameters,
			one to specify just the temperature value (defaults to
				type Celsius),
			one to specify just the type (defaults to 0 for the
				temperature value,
			and the default that specifies neither (defaults to
				0 degrees Celsius).
		Three "write" methods
			one to display both parameter values,
			one to display temperature in degrees C,
			and one to display temperature in degrees F.
		Two methods to get (return) temperature, one in degrees C
			and the other in degrees F.
		Three methods to set the parameters, either one at a time,
			or both together.  All are named "set" but the method
			is overloaded for the three variations of parameters.
